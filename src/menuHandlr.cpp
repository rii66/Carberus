...
void handleMenu(int direction, bool pressed) {
        ...
        if (holdTime > 50 && !longActionDone) {
            ...
            if (!inMenu) {
                ...
                if (activeStation == STATION_HOTAIR) {
                    ...
                    // Short click Hot Air:
                    // TEMP <-> FAN selection
                    hotAirFanEdit = !hotAirFanEdit;
                    beepSelect();
                }

                else {

                    // Short click Solder = Boost
                    startBoost();
                }
            }
    ...
    if (!inMenu) {
        ...
        if (activeStation == STATION_HOTAIR) {
            ...
            // Hot Air: encoder edits TEMP or FAN
            if (hotAirFanEdit) {

                int fan = airGetFan() + (direction * 5);
            ...
            // Solder: encoder edits target temperature
